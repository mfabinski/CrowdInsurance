...
RUN cd ../SmartInsurance && sed -i 's|^\s*service\.url.*=.*;|service.url = location.origin;|' www/Module/apiendpoint.js && npm install bower -g --loglevel warn && npm install --loglevel warn && bower install --allow-root -s
EXPOSE 3000
CMD /bin/sh npmstart
