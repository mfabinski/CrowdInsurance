...
exports.parameterSchadensfall = function(req,res,next){
  ...
  if(versicherungID == undefined || schadenshoehe == undefined){
    res.status(400).send("Kann versicherungID und/oder schadenshoehe nicht lesen.");
    return;
  }
...
exports.obVersicherungSchonVollIst = function(req,res,next){
  ...
  tdg.selectInvestitionsSumVonVersicherung(versicherungID,
      function(data){
        ...
        var investitionswert = internal.moneyToNumber(req.body.investitionswert);
        if(diff >= investitionswert){
          next();
        } else if(diff > 0 && diff < investitionswert) {
          // Investiere maximal wenn es nicht reicht
          req.body.investitionswert = internal.numberToMoney(diff);
          next();
        } else if(diff == 0){
